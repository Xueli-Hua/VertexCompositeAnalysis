...
class HiEventPlaneHFbyPF : public edm::one::EDAnalyzer<edm::one::WatchRuns> {
public:
  explicit HiEventPlaneHFbyPF(const edm::ParameterSet&);
  ~HiEventPlaneHFbyPF();
...
private:
  ...
  TH1D *hpfe, *hpfet, *hpfet_plus, *hpfet_minus, *hpfetSum, *hpfetSum_plus, *hpfetSum_minus;
  ...
  edm::EDGetTokenT<pat::PackedCandidateCollection> pfCandidate_;
...
HiEventPlaneHFbyPF::HiEventPlaneHFbyPF(const edm::ParameterSet& iConfig) :
  patCompositeCandidateCollection_Token_(consumes<pat::CompositeCandidateCollection>(iConfig.getUntrackedParameter<edm::InputTag>("VertexCompositeCollection")))
{
  ...
  pfCandidate_ = consumes<pat::PackedCandidateCollection>(iConfig.getUntrackedParameter<edm::InputTag>("pfCandidateSrc"));
...
HiEventPlaneHFbyPF::~HiEventPlaneHFbyPF()
{
...
void
HiEventPlaneHFbyPF::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
  //check event
...
void
HiEventPlaneHFbyPF::fillRECO(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
  //get collection
  ...
  if(!vertices.isValid()) throw cms::Exception("HiEventPlaneHFbyPF") << "Primary vertices  collection not found!" << std::endl;
  ...
  if(!v0candidates.isValid()) throw cms::Exception("HiEventPlaneHFbyPF") << "V0 candidate collection not found!" << std::endl;
  ...
  if(candSize>MAXCAN) throw cms::Exception("HiEventPlaneHFbyPF") << "Number of candidates (" << candSize << ") exceeds limit!" << std::endl; 
  ...
  // EP reconstruction by PF candidates
  edm::Handle<pat::PackedCandidateCollection> pfCandidates;
  iEvent.getByToken(pfCandidate_, pfCandidates);
  ...
  for (const auto& pfcand : *pfCandidates) {
    if (pfcand.pdgId() == 1 || pfcand.pdgId() == 2){ // HF tower identified as a hadron || HF tower identified as an EM particle
      const bool eta_plus = (pfcand.eta() > 3.0) && (pfcand.eta() < 6.0);
      const bool eta_minus = (pfcand.eta() < -3.0) && (pfcand.eta() > -6.0);
      //if (pfcand.et() < 0.01 || pfcand.et() > 30) continue; // default: pfcand.et() < 0.0
      if (eta_plus || eta_minus)
      {   
        double pfe = pfcand.energy();
        double pfet = pfcand.et();
        double pfphi = pfcand.phi();
        pfqx += pfet*cos(2*pfphi);
        pfqy += pfet*sin(2*pfphi);
        pfEt += pfet;
        ...
        if(eta_plus){
          double pfet_plus = pfcand.et();
          double pfphi_plus = pfcand.phi();
          pfqx_plus += pfet_plus*cos(2*pfphi_plus);
          pfqy_plus += pfet_plus*sin(2*pfphi_plus);
          pfEt_plus += pfet_plus;
          hpfet_plus->Fill(pfet_plus);
        }
        else if(eta_minus){
          double pfet_minus = pfcand.et();
          double pfphi_minus = pfcand.phi();
          pfqx_minus += pfet_minus*cos(2*pfphi_minus);
          pfqy_minus += pfet_minus*sin(2*pfphi_minus);
          pfEt_minus += pfet_minus;
          hpfet_minus->Fill(pfet_minus);
        }
      }
    }
  }
  ...
  hpfetSum->Fill(pfEt);
  hpfetSum_plus->Fill(pfEt_plus);
  hpfetSum_minus->Fill(pfEt_minus);

  pfQx = pfqx/pfEt;
  pfQy = pfqy/pfEt;
  pfQx_plus = pfqx_plus/pfEt_plus;
  pfQy_plus = pfqy_plus/pfEt_plus;
  pfQx_minus = pfqx_minus/pfEt_minus;
  pfQy_minus = pfqy_minus/pfEt_minus;
}
...
{ 
  ...
  if(doRecoNtuple_)
  {
    ...
    PATEventPlaneNtuple->Branch("pfQx",&pfQx,"pfQx/D");
    PATEventPlaneNtuple->Branch("pfQy",&pfQy,"pfQy/D");
    PATEventPlaneNtuple->Branch("pfQx_plus",&pfQx_plus,"pfQx_plus/D");
    PATEventPlaneNtuple->Branch("pfQy_plus",&pfQy_plus,"pfQy_plus/D");
    PATEventPlaneNtuple->Branch("pfQx_minus",&pfQx_minus,"pfQx_minus/D");
    PATEventPlaneNtuple->Branch("pfQy_minus",&pfQy_minus,"pfQy_minus/D");
  } // doRecoNtuple_

}

void
HiEventPlaneHFbyPF::initHistogram()
{
  ...
  hpfe =  fs->make<TH1D>("hpfe",";PF E", 5000, 0,50.);
  hpfet =  fs->make<TH1D>("hpfet",";PF Et", 3000, 0,30.);
  hpfet_plus =  fs->make<TH1D>("hpfet_plus",";PF plus Et", 3000, 0,30.);
  hpfet_minus =  fs->make<TH1D>("hpfet_minus",";PF minus Et", 3000, 0,30.);

  hpfetSum =  fs->make<TH1D>("hpfetSum",";PF SumEt", 3000, 0,3000.);
  hpfetSum_plus =  fs->make<TH1D>("hpfetSum_plus",";PF plus SumEt", 3000, 0,3000.);
  hpfetSum_minus =  fs->make<TH1D>("hpfetSum_minus",";PF minus SumEt", 3000, 0,3000.);
}


//--------------------------------------------------------------------------------------------------
void 
HiEventPlaneHFbyPF::beginRun(const edm::Run& iRun, const edm::EventSetup& iSetup)
{
}
...
//loop  ------------
void 
HiEventPlaneHFbyPF::endJob()
{
}

//define this as a plug-in
DEFINE_FWK_MODULE(HiEventPlaneHFbyPF);
