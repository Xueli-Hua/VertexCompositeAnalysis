...
PATEventPlaneTrack::fillRECO(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
  ...
  auto DauMu = std::make_unique<std::vector<pat::Muon>>();
  ...
  { 
    ...
    {
      const pat::Muon& dau = *(trk.daughter(iDau));
      DauMu->push_back(dau);
      /*
  ...
  for(unsigned it=0; it<trackColl->size(); ++it){
	...
	htrk->Fill(track->eta(),track->pt());
    	...
    	for (std::vector<pat::Muon>::const_iterator muon = DauMu->begin(); muon < DauMu->end(); muon++) {
